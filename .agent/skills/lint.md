...
## 流程
1. **扫描（只读）**：
   - 孤儿页：`wiki/` 下无任何入链、也不被 `_overview.md` 或他处引用的笔记。
   - 缺失链接：笔记里 `[[...]]` 指向不存在的文件。
   - 矛盾说法：不同笔记对同一概念描述冲突（尤其 `status: disputed`）。
   - 过时内容：引用了被新资料淘汰的说法（对照 raw/ 与 `updated` 日期）。
...
## Gotchas（真实踩过的坑）
- **默认只读优先**：先报告，用户说"直接修"才改。自动修改曾引入意外破坏。
- **不要把指向 raw/ 的链接当"断链"修掉**：raw 是本地引用，全新 clone 后才断，属设计如此，不是 bug。
- **绝不删历史 commit**；删除用冷归档（`.agent/archive/`）。
