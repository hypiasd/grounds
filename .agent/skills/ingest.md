...
## 目标（完成时仓库应处于的状态）
- `raw/` 有该资料（或一份引用页）；`wiki/<topic>/` 下有摘要页，通过 `sources:` 指回 raw；互链建立；`_overview.md` 同步；已 `git commit`。

## 流程
1. **落地 raw/**：用户已手动放入 → 直接用；给链接 / 要求下载 → 下载或抓取文本存 `raw/`（文件名小写中划线）；给仓库 → 默认写引用页（URL + 关键说明 + 关注的文件），若你明确要存源码快照再落地。
