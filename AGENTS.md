...
**触发**："考考我"、"grill me on X"、"质询一下"、"来考我"

**流程**：确认目标概念 → 读相关笔记做功课 → 一次一个 Forcing Question（附带推荐答案）→ 根据回答扎实度决定深挖还是换方向 → 收尾总结强弱项

**产出**：不写文件。暴露的理解盲区和洞见可提示走 learn 或 capture

**注意**：用户说"不知道"时追问而非开讲；一次只问一个问题。
