...
sources:
  - 翁家翌：OpenAI，GPT，强化学习，Infra，后训练，天授，tuixue，开源，CMU，清华｜WhynotTV Podcast #4.pdf
---

## 引言：走进翁家翌的世界

本期播客嘉宾翁家翌，2022年加入 OpenAI，是 GPT-3.5、GPT-4、GPT-4o 到 GPT-5 一系列核心模型背后的核心贡献者之一。他的核心贡献可以总结为三个关键词：**强化学习（RL）、Post-Training、Infra**。他搭了整个 OpenAI 的 Post-Training RL Infra，因此几乎所有 model release 上都有他的名字。

[图：figures/talk_opening.jpg — 见 PDF]
*播客开场画面 *

*视频画面时间区间：00:01:01--00:01:12。*

在加入 OpenAI 之前，翁家翌就已经用开源和产品影响过无数人：从清华开源作业与资料试图打破信息差，到开源强化学习框架**天授（Tianshou）**，再到免费签证查询系统**tuixue.online**。他把代码和工具视作一种**慈善**——追求的不是掌声，而是 impact。

> **[重要]** **翁家翌的核心画像**
> - **角色**：OpenAI 核心研究员，Post-Training RL Infra 负责人
> - **关键词**：强化学习、Post-Training、Infra、开源、Impact
> - **代表性项目**：天授（Tianshou）RL 框架、tuixue.online 签证查询系统
> - **教育背景**：清华本科 → CMU 硕士
> - **核心哲学**：打破信息差、追求 Impact 而非声望、工程能力 > 研究能力
...
> 翁家翌区分了两种"外部评价"：
>
> - **既有评价体系**（GPA、高考、PhD > Master）：他不喜欢，因为它是一套僵化的标准，推着人走。
> - **共识评价**（GitHub star、点击量、citation、inference 次数）：他认可，因为这是"每个人发自内心的点赞"，代表真正的 impact。
>
...
> **[知识]** **开源的意义**
> 翁家翌对开源的看法不是"技术炫耀"或"社区贡献"，而是一个更根本的信念：**知识不应该成为特权的护城河**。他的开源行为本质上是对"信息差"这种社会不平等的对抗——哪怕这种对抗在当时来看是"自毁长城"。

### 天授（Tianshou）：两周写完的 RL 框架

2020年初，翁家翌想把之前写的 RL 实验代码整合成一个框架。他先看了当时最流行的 RLlib，花了一个月——

> 太复杂了，抽象太多了。一个 RLlib 快几十万行代码，完全不被人接受。我根本不知道我要改的东西该怎么实现。

于是他决定推倒重来。**两周**，一个人，写出了天授的第一版。

> **[重要]** **天授成功的关键**
> 1. **抓住用户需求**：Researcher 需要"好用、好改"的 RL 框架，天授做到了。
> 2. **一致性优先**：一个人写，顶层设计一致，代码短，抽象清晰。
> 3. **修改点唯一**：如果要在天授上加 feature，只有一个地方能改——设计本身就规定了修改路径。
> 4. **应用于心**：每个算法实现不超过20行，对着 paper 实现就好。
...
> **[注意]** **项目腐化的根源：不一致性**
> 翁家翌指出了大多数开源项目腐化的核心原因：**多人在不了解彼此假设的情况下各自写代码**，导致复制粘贴、代码膨胀、抽象泄漏。一致性比代码行数、测试覆盖率等任何工程指标都更重要。好项目从头到尾都是一致的。

### tuixue.online：签证查询系统

在疫情最严重的时候，他做了另一个"慈善"项目——`tuixue.online`，一个免费签证查询系统，帮助无数留学生追踪签证状态。同样是 non-profit，同样是出于"做一些对大家有意义的事"的冲动。
...
- **名望（fame）**：坐上了某个位置，有头衔有身份，但对别人可能毫无意义甚至负面。
- **Impact**：做了对别人有用的事，被真正受益的人记住。

> 我想做的是力所能及地对身边的人好，做一些对大家有意义的事。这个度量标准是：让更多的人记得你。

但他也清醒地认识到：**这个标准只对自己适用**，不会拿它去要求别人。"如果你发现自己的标准成了新的枷锁——你可以改。你不是这个标准的奴隶。"
...
> 翁家翌的核心判断：
>
> - **PhD 培养的是学术能力**：写 paper、讲故事、画图、宣发。
> - **工业界需要的是工程能力**：建 infra、快速验证 idea、保证正确性。
> - **差异化竞争**：让公司愿意招"master 的你"而不是"另一个 PhD"——你必须有 PhD 候选人所不具备的东西。
...
1. **Idea 非常便宜**：找人讨论一下就有了。真正有研究直觉的人（如 Alec Radford）动脑子比普通 PhD 动脑子有用得多。
2. **验证 Idea 才是瓶颈**：你单位时间内能验证多少有效的 idea，并且保证验证结果是正确的。
3. **验证依赖 Infra**：Infra 有 bug → 验证结果不可靠 → 研究方向被误导。
4. **谁修 bug 谁赢**：**每家的 Infra 都有不同程度的 bug，谁修 bug 越多，谁的模型训得越好。**
...
>
> - **分子**：迭代速度（iteration speed）
> - **分母**：Infra 正确性（有多少 bug 没修）

他进一步指出，目前所有大模型公司的 Infra"都有 bug"，差距只是多少的问题。而"修 bug"这件事不能外包给纯 researcher——他们"不愿意修也不想修，因为这不在他们的评价体系里"。

> **[知识]** **Infra 工程师 vs Researcher 的分工**
> - **Researcher**：找人讨论 → 产生 idea → 改 flag 跑实验 → 看结果。不需要了解 Infra 底层。
> - **Infra Engineer**：设计 distributed training 架构、修 bug、保持一致性、优化吞吐量。
> - **正确的关系**：Researcher 提需求，Infra Engineer 建底座。底座越稳，researcher 越能专注于 idea 验证。
...
1. **Infra 稳定性**：工业级 RL 训练涉及多个模型（policy model、reward model、reference model），分布式训练的一致性和容错是巨大挑战。
2. **规模扩展**：从实验室的小规模实验扩展到 GPT 级别的模型，每一层抽象都可能断裂。
3. **反馈质量**：人类标注的质量和一致性直接影响 RLHF 效果——"垃圾 reward 只会训练出垃圾 policy"。
...
- **吞吐量**：单位时间能处理多少训练样本和奖励信号。
- **正确性验证**：如何确保分布式训练中每一步的结果都是可靠的。
- **Context 一致性**："人多了，context inconsistent，每个人都会写一些奇怪的东西"——这是大型 Infra 项目的自然熵增。
- **迭代周期**：从改代码到看到实验结果的时间间隔越短越好。
...
> **[重要]** **翁家翌的内部视角**
> - **原因**：Ilya 和其他董事会成员不信任 Sam，投票把他投出去了。
> - **底层员工的反应**：非常震惊（surprise），完全不知道发生了什么。
> - **董事会缺乏透明度**：董事会之前对底下的人是缺乏透明度的，决策过程不透明。
> - **Sam 为什么能回来**：因为很多员工觉得"如果纯技术出身的人领导，可能没有那么多资源"——AGI 的实现不只是技术问题，还需要融资、商业、资源整合。

> 你可以把 Sam 抽象成一个 Persona，这个 Identity 乘以一个系数。别人对这个 Identity 的认同感如果缺失了，组织就运转不起来。这个短时间内 AI 替代不了。

### 人才流失：一个健康的组织没有人是不可替代的
...
这意味着：**OpenAI 今天做的东西，外面的人也不是说难如登天、不能复刻。** 因为没有黑魔法——"把最简单的东西做好就好了"。
...
> **[重要]** **真正让 OpenAI 警觉的事**
> {
> 不是 DeepSeek 的模型在哪个榜单上比 GPT 分高——OpenAI "很长时间以来都没有特地为了刷 benchmark 做什么事情"。**真正让他们警觉的是 DeepSeek 的迭代速度**——也就是 Infra 的 cycle time。
> }

但他也坦言：OpenAI 目前在这个指标上**不是世界第一**。
...
> **[知识]** **大公司 vs 小团队的速度悖论**
> - **小团队优势**：代码库小、context 一致、沟通成本低 → 迭代快
> - **大公司优势**：用户反馈多、资源充裕、多 use case 测试 → 鲁棒性强
> - **翁家翌的判断**：这个 trade-off 是"人类组织发展到一定规模后必然会面对的问题，无法避免"
> - **一个可能的解法**：拥有无限 context 的 AI Agent 来负责组织的信息共享和决策——替代人类 CEO
...
他将 OpenAI 的使命拆成两步：**先实现 AGI，再造福全人类**。而"造福全人类"目前的理解是做产品、以尽可能便宜的价格让普通人接触到技术（如免费的 ChatGPT），而不是直接丢一个裸权重出去。
...
> 两个核心主张：
>
> 1. **世界是确定性的**：所有东西都可以被预测。你现在脑子里在想什么，下一个单词说什么，全都是宇宙大爆炸那一刻就定好了——这是一个确定性的马尔可夫过程。
> 2. **人是没有自由意志的**：他声称这是"验证过无数遍"的结论。
...
他提到最想用 AI 解决的难题是**预测未来**——"我个人对于造一个世界还是很有吸引力的。你需要提前生成剧本。"
...
1. **Impact > Fame**：追求实质影响而非空名。
2. **工程 > 研究**：在 AI 当前阶段，工程能力（建 Infra、修 bug、提吞吐）比研究能力（发 paper、写故事）更重要。
3. **迭代速度 = 核心竞争力**：谁的 Infra 迭代快、bug 少，谁的模型就训得好。
4. **信息差应该被打破**：知识不应成为特权护城河。
5. **世界可能是确定性的**：但最佳策略是"忘掉它，去体验"。
...
> **[重要]** **翁家翌式 AI 人才观**
> - **差异化定位**：Master 要和 PhD 竞争，靠的不是"更像 PhD"，而是"PhD 做不到的事"——工程能力。
> - **够用就行**：在评价体系中满足最低门槛，把剩余精力投入自己真正在意的方向。
> - **开源是价值观**：不是社区运营策略，而是对信息不对称的对抗。
> - **Infra 是硬通货**：在这个阶段，能修 bug 的人比能提 idea 的人更稀缺。

> **[重要]** **翁家翌式 AI 竞赛观**
> - **不追 benchmark**：OpenAI 不关心谁在某榜上比 GPT 高多少分。
> - **追迭代速度**：真正让 OpenAI 警觉的是 DeepSeek 的 Infra 迭代效率。
> - **大公司的宿命**：规模带来 context 不一致和臃肿——这是所有组织发展到一定阶段的必然结果。
> - **出路可能是 AI Agent**：一个拥有无限 context 的 agent 来解决人类组织的协调问题。

### 扩展思考

翁家翌的对话提出了几个值得深思的开放问题：

1. **Infra 工程师 vs Researcher 的分工是否可持续？** 当 Infra 越来越复杂，Researcher 对底层的了解越来越浅，两者之间的 gap 会不会导致研究方向被 Infra 的局限性所绑架？
2. **"评价体系"的重构**：翁家翌反对 GPA/paper 这类既有评价体系，但提倡 GitHub star/用户量这类"共识评价"。问题是：当共识评价也成为主流后，会不会同样腐化？他自己的回答是"你可以改自己的标准"——但这需要持续的自我觉察。
3. **开源 vs 闭源的博弈论困境**：OpenAI 的闭源策略在当前市场环境下是理性的，但长期来看会不会削弱整个 AI 生态的进步速度？DeepSeek 的 open weights 是否会改变这个博弈格局？
4. **AGI 的"out-of-distribution"难题**：翁家翌认为 AGI 的瓶颈不在于已有能力的泛化，而在于处理训练数据中占比极低的领域（如 Infra 代码）。这暗示了一个 deeper problem：AGI 需要的不是更好的 scaling，而是某种根本性的架构创新。

### 拓展阅读

- 天授（Tianshou）RL 框架：[github.com/thu-ml/tianshou](https://github.com/thu-ml/tianshou)
- tuixue.online 签证查询系统：[tuixue.online](https://tuixue.online)
- OpenAI Post-Training 相关技术博客：[openai.com/research](https://openai.com/research)
- John Schulman 关于 RLHF 的演讲与论文
- DeepSeek 技术报告：了解竞争对手的 Infra 设计与迭代效率
