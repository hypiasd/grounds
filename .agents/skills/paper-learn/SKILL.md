...
3. **校验（必做）**：
   ...
   - `git status` 确认改动符合预期
4. **提交**：先 `git status` 确认仅本笔记 + `index.md` 改动；显式 `git add "paper/<topic>/<论文标题>.md" "paper/<topic>/index.md"`（用仓库根相对路径 + 引号，论文标题常含空格/中文/特殊字符；不要 `git add -A`，避免误带其他未完成改动）；`git commit -m "paper-learn <topic>: <一句话>" && git push`。
