...
- 仓库新增一篇 `paper/<topic>/<论文标题>.md`（或更新已有笔记），已 `git commit`（**仅 grounds 才 push**）。
...
which pdftoppm                           # 扫描版 PDF 转图片（ocrmypdf 不可用时的回退路径）
which gh                                 # GitHub CLI（论文-代码对照时可选，缺失时走 WebSearch）
```

- `pdfinfo` / `pdftotext` 缺失 → **必需**，缺失时报告用户安装（`brew install poppler`）后退出，不要硬抠 PDF
- `qpdf` / `ocrmypdf` / `pdftoppm` 缺失 → 仅在 PDF 加密/扫描版时需要；`ocrmypdf` 缺失时回退到 `pdftoppm` + `tesseract` 逐页 OCR；`pdftoppm` 也缺失时报告用户安装 poppler
...
1. **获取论文**：
   ...
   - 下载命令示例：
     ```bash
     # arxiv：加 --max-time 防止大文件下载卡死，--retry 兜底网络抖动
     curl -L --max-time 180 --retry 2 -o raw/papers/1706.03762.pdf "https://arxiv.org/pdf/1706.03762"
     ```
...
3. **提取 metadata**：标题 / 作者 / venue / 年份 / arxiv_id。
   - **arxiv 论文**（推荐）：查 arxiv API 获取结构化 metadata（venue 难自动提取，需手动从 PDF 首页读，见下方"venue 难自动提取"）。返回的 XML 中**第一个 `<title>` 是 feed title（如 "arXiv Query: ..."），第二个才是论文 title**；`<summary>` 标签是 abstract；`<name>` 是作者。
     ```bash
     # 必须用 https:// + -L 跟随重定向，http:// 会静默返回 0 字节
     # -f 在 HTTP 错误时返回非零 exit code，避免把网络错误当"论文不存在"
     # --max-time 30 防止 arxiv API 偶发卡住（API 响应应 < 5s）
     # 用 python 解析 XML 比 grep 稳健（避免双 <title> 干扰、自动去标签）
     curl -fsL --max-time 30 "https://export.arxiv.org/api/query?id_list=<arxiv_id>" -o /tmp/arxiv_<id>.xml \
       && python3 -c "
...
" || echo "arxiv API fetch failed"
   ...
   - **venue 难自动提取**：先从 PDF 首页/footnote 找会议标识（如 "Published in NeurIPS 2024"）；找不到时分 venue 查询：
...
3. **校验（必做）**：
   ...
   - `git status` 确认改动符合预期
4. **提交**：先 `git status` 确认仅本笔记 + `index.md` 改动；显式 `git add "paper/<topic>/<论文标题>.md" "paper/<topic>/index.md"`（用仓库根相对路径 + 引号，论文标题常含空格/中文/特殊字符；不要 `git add -A`，避免误带其他未完成改动）；`git commit -m "paper-learn <topic>: <一句话>"`。**仓库名判定（与 sync 一致）：是 grounds 才 `git push`，临时派生仓无 origin 不 push，靠 `$sync` 推回。**

---

## paper 笔记模板

frontmatter 字段命名沿用 conventions.md 惯例（`title/topic/tags/summary/created/updated/sources`），但**正文结构不套用 conventions.md，按本 SKILL.md 模板走**（conventions.md 是 wiki 笔记规范）。`sources` 字段 wiki 和 paper 都可用（conventions.md 已定义为可选字段），paper 笔记里指向 `raw/papers/` 下的源 PDF（注意：`raw/papers/` 受 .gitignore 保护不进 git，跨机器需重新下载 PDF，sources 链接仅本机有效）。
...
- **讲完忘收口**：commit 之后——是 grounds 必须 `git push`（否则换机器看不到）；临时派生仓只 commit，靠 `$sync` 推回 grounds。
