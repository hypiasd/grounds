...
- **概念 / 认知（教学向，用户追问"为什么 / 怎么理解"时）**：统一命名 `**概念 <主题>（YYYY-MM-DD）**`，内附类比 / 形式化 / 图示；区别于「问题 / 解决」块（坑是"出错"，概念是"认知升级"，**勿混用前缀**）。同一节点概念洞察 >5 条时，改用带日期的 `### <YYYY-MM-DD> <主题>` 子小节（仍留同节点内，不另开文件），保持可扫读。
- **结果**（验证节点）：触发方式 / 结果 / 备注。
...
> **教学向项目（用户在学概念、反复追问"为什么"）的洞察，勿塞进「问题 / 解决」块**——它不是坑。改用上文 M0 的「概念 / 认知」块，统一命名 `**概念 <主题>（YYYY-MM-DD）**`，避免一个节点出现 `概念洞察 / 代码细节洞察 / 可视化 / HBM 流量直白版` 等数种不一致前缀（真实案例：vllm-plus M0 单节点曾累积 8 种即兴前缀）。
