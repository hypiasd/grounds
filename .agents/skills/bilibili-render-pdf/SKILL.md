...
> **前置条件**：本节仅在已存在工作目录时执行。首次运行（`raw/videos/` 下无匹配标题的目录）直接跳到「工作区设置」创建新目录。
>
> **如何知道上次运行的工作目录**：先 `ls raw/videos/` 列已有工作目录，匹配标题决定是否复用。同一视频重跑时直接复用同名目录，然后 `cd` 到该目录再执行本节检查。在仓库根目录执行时，这些相对路径全部不存在，检查恒为 MISSING。
...
2. 检测分P 视频。列出所有 P 并问用户处理哪些 P。**多 P 时每个 P 独立工作目录** `raw/videos/<标题>-part<n>/`（与 youtube-render-pdf 统一命名），各自产出独立 PDF，互不干扰。单 P 时工作目录就是 `raw/videos/<标题>/`。
