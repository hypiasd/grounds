...
本 skill 使用 `uv` 管理 Python 依赖。首次运行前进入 `.agents/skills/video-render-pdf/` 并执行：

```bash
uv sync
```

这会创建 `.venv/` 并安装 `yt-dlp`、`faster-whisper`、`openai`、`torch` 等核心依赖。之后所有 Python/uv run yt-dlp 命令都通过 `uv run` 在该隔离环境中执行。

> **注意**：当前运行环境（TRAE）会注入 `PYTHONHOME`/`PYTHONPATH`，可能污染 venv。因此所有 `uv run` 命令前都加 `env -u PYTHONHOME -u PYTHONPATH`，确保使用 skill 自己的 `.venv`。

### 1. 硬件检查

```bash
env -u PYTHONHOME -u PYTHONPATH uv run python -c "import torch; gpu = torch.cuda.is_available() or torch.backends.mps.is_available(); print('GPU available:', gpu)" 2>/dev/null || echo "torch not available"
...
```bash
env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --version          # yt-dlp（通过 uv 管理）
env -u PYTHONHOME -u PYTHONPATH uv run python -c "import faster_whisper; print('ok')"  # faster-whisper: CTranslate2 后端，CPU 快 3-5×
which whisper 2>/dev/null || echo "openai-whisper CLI optional"  # OpenAI Whisper CLI（可选，uv 默认不装）
...
[ -f sources/subtitles.srt ] && env -u PYTHONHOME -u PYTHONPATH uv run python -c "
...
1. 先检查视频元数据。优先标题、章节、时长、封面可用性、字幕可用性。
   ...
   ```bash
   env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --print "%(title)s|%(description)s|%(duration)s|%(thumbnail)s|%(chapters)s|%(subtitles)s" --skip-download "<URL>"
   ```
...
```bash
# 从元数据取标题并清洗（注意：TRAE 注入的 PYTHONHOME/PYTHONPATH 会污染 venv，需先清空）
TITLE="$(env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --print "%(title)s" --skip-download "<URL>" | sed 's/[/:?*"<>|]/-/g')"
...
- **[B 站]**：优先 `zh-Hans`、`zh-CN`、`zh`、`ai-zh` 轨。B 站 metadata 经常误报 `NA` 即使 CC 实际可下载（ai-zh 轨观察到），**无论元数据结果如何都要尝试 CC 下载**：
  ```bash
  env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --cookies-from-browser chrome --write-subs --sub-langs "zh-Hans,zh-CN,zh,ai-zh" --convert-subs srt \
  ...
  ```bash
  env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --write-subs --sub-langs "zh-Hans,zh-CN,zh,ai-zh" --convert-subs srt \
    --skip-download -o "sources/subtitles.%(ext)s" "<URL>"
  mv sources/subtitles.*.srt sources/subtitles.srt 2>/dev/null
  ```
...
- **[YouTube]**：优先 `zh-Hans`、`zh-CN`、`zh`、`en` 轨，用 `--write-auto-subs` 同时拿自动字幕；CC 通常可用，可先 `--print subtitles` 检查，若 `NA` 再回退 Whisper：
  ```bash
  env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --cookies-from-browser chrome --write-subs --write-auto-subs --sub-langs "zh-Hans,zh-CN,zh,en" --convert-subs srt \
    --skip-download -o "sources/subtitles.%(ext)s" "<URL>"
  mv sources/subtitles.*.srt sources/subtitles.srt 2>/dev/null
  ls -la sources/subtitles.srt
  ```
...
2. 按自适应时长切段，逐段调用 API，组装 SRT（脚本 `.agents/skills/video-render-pdf/scripts/api_transcribe.py`）。chunk size 自动计算：`min(10, max(3, duration_sec // 20))` 分钟——短视频 3 分钟/chunk 保留精细时间戳，长视频 10 分钟/chunk 减少 API 调用次数。也可手动指定：`env -u PYTHONHOME -u PYTHONPATH uv run python scripts/api_transcribe.py audio.wav out.srt 600`。
...
1. 提取音频为 WAV：
   ```bash
   env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp -x --audio-format wav -o "sources/audio.%(ext)s" "<URL>"
   ```
...
2. 按上面的[策略表](#4-转录策略选择)选工具和模型。
   ...
   **关键：流式写入 SRT，每 segment 立刻追加 + flush**，不要收集到数组末尾再一次性写——否则转录全程 SRT 都不存在，agent 会误判为卡死。
   ...
   env -u PYTHONHOME -u PYTHONPATH uv run python transcribe.py sources/audio.wav sources/subtitles.srt small
   ```
...
2. 批量评估一个片段的所有候选，只保留 top-ranked 帧：
   ...
   env -u PYTHONHOME -u PYTHONPATH uv run python "$(git rev-parse --show-toplevel)/.agents/skills/video-render-pdf/scripts/frame_assess.py" \
...
2. 优先最高可用视频源做图提取。探查格式选当前环境实际可下载的最高分辨率：
   ```bash
   env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp -F "<URL>"  # 列格式
   ```
   **[B 站]** 1080P+ 通常需登录 cookies；**[YouTube]** 年龄限制内容需 cookies。720P 在 1920×1080 显示器上做图提取通常足够。

3. 下载视频用于帧提取：
   ```bash
   env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp -f "bestvideo[height<=720]+bestaudio" --merge-output-format mp4 -o "sources/video.mp4" "<URL>"
   ```

   **纯口头内容捷径**：当「视频类型预判」判定为纯口头内容（访谈/播客/圆桌）时，完整视频下载性价比极低（120 分钟 ≈ 200MB 只为抽 2 帧）。改用按需下载：
   ```bash
   # 先拿音频（ASR 必需）和封面
   env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp -f bestaudio -o "sources/audio.%(ext)s" "<URL>"
   curl -L -o cover.jpg "<thumbnail_url>"
   # 帧只下载需要的几秒（如开场 30s + 中点 5s），video.mp4 裁剪后留 sources/
   env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --download-sections "*30-35" -f "bestvideo[height<=720]+bestaudio" \
...
| `.pdf` 文件 | `./<basename>.pdf` | 编译 PDF（跑两次 `xelatex` 出 TOC 和交叉引用；本地阅读用，可保留或删除） |
...
video/ 下的 .tex 在网页上无法渲染，需要转成 .md 给前端渲染。用 skill 自带的脚本 `.agents/skills/video-render-pdf/scripts/tex_to_md.py`：

```bash
# 在工作目录下执行；输出文件名必须是 index.md（Quartz folder note 约定）
env -u PYTHONHOME -u PYTHONPATH uv run python "$(git rev-parse --show-toplevel)/.agents/skills/video-render-pdf/scripts/tex_to_md.py" \
...
- **Shell 管道截断视频标题**：`$(yt-dlp --print title ... | sed ...)` 中的 `|` 会被 Shell 解释为管道而非传给 sed。当标题含 `|` 时，用 Python 清洗标题更可靠：`TITLE=$(env -u PYTHONHOME -u PYTHONPATH uv run python -c "import sys; t=sys.argv[1]; print(''.join(c if c not in '/:?*\"<>|' else '-' for c in t))" "$(env -u PYTHONHOME -u PYTHONPATH uv run yt-dlp --print "%(title)s" --skip-download "<URL>")")`。
