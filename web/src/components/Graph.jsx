    ...
    >
      <g className="zoom-layer">
        ...
        {nodes.map(node => {
          ...
          const url = node.type === 'tag'
            ? null
            : `/grounds/${node.id.replace('/_overview', '')}`

          return (
            <g
              key={node.id}
              transform={`translate(${node.x},${node.y})`}
              style={{ cursor: url ? 'pointer' : 'default', opacity }}
            ...
            >
              ...
              {!mini && (
                <text
                  ...
                  textAnchor="middle"
                  fontSize={node.type === 'tag' ? 9 : 10}
                  fontFamily="var(--font-heading)"
                  fill="var(--text-secondary)"
                  pointerEvents="none"
                >
                  {node.type === 'tag' ? node.title : node.title.length > 15 ? node.title.slice(0, 13) + '...' : node.title}
                </text>
              )}
            </g>
          )
