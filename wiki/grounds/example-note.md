...
- **Q**: Self-Attention 和 Cross-Attention 的区别是什么？各用在什么场景？
  **A**: Self-Attention 的 Q、K、V 来自同一个序列，让序列内部的元素互相"看见"彼此，用于编码器（如 BERT）和自回归解码器的前序部分。Cross-Attention 的 Q 来自一个序列（如解码器），K、V 来自另一个序列（如编码器输出），用于解码器关注编码器的输出（如机器翻译）。关键区别：Self-Attention 是"自己看自己"，Cross-Attention 是"我看你"。
