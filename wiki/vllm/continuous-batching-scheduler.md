...
updated: 2026-07-14
sources:
  - ../../raw/wiki/vllm/vllm/v1/core/sched/scheduler.py
---
...
- **抢占行为**（[scheduler.py:get_preempt_request](../../raw/wiki/vllm/vllm/v1/core/sched/scheduler.py)）：释放请求的**所有 KV block**，`num_computed_tokens` 重置为 0，塞回 waiting 队列头部
...
- 源码：[scheduler.py](../../raw/wiki/vllm/vllm/v1/core/sched/scheduler.py) — `schedule()`（L420）和 `_preempt_request` 方法
